...
class MutableNetworkABC(ImmutableNetworkABC[NodeT, LinkT, NodeTypeT, LinkTypeT], ABC):
    ...
    def add_nodes(
        self, nodes_to_add: Mapping[NodeId, NodeT] | Collection[NodeT]
    ) -> None:
    ...
    def add_links(
        self,
        links_to_add: (
            Collection[tuple[EndNodeIdPair, LinkT]] | Mapping[EndNodeIdPair, LinkT]
        ),
    ) -> None:
        if isinstance(links_to_add, dict):
            return _add_links(self, links_to_add.items())
        return _add_links(self, links_to_add)

    def append_(
        self, network_to_append: ImmutableNetworkABC[NodeT, LinkT, NodeTypeT, LinkTypeT]
    ) -> None:
    ...
    def replace_node(
        self, index: NodeIndex, updated: NodeT, renamed: bool = False
    ) -> None:
    ...
    def remove_isolated_nodes(self) -> None:
        self._underlying_digraph.delete_vertices(
            self._underlying_digraph.vs.select(_degree=0)
        )

    def __add__(self: Self, other: Self) -> Self:
        return self.__class__(
            self._underlying_digraph.union(other.underlying_digraph, byname=True),
            self.time_context,
        )

    def sub_network(
        self: Self, selected: Collection[NodeIndex] | Collection[NodeId]
    ) -> Self:
        return self.__class__(
            self._underlying_digraph.subgraph(selected), self.time_context
        )

    def delete_nodes_with_type(self, types: AbstractSet[NodeTypeT]) -> None:
        self._underlying_digraph.delete_vertices(
            [i for i, n in enumerate(self.all_nodes) if n.node_type in types]
        )

    def delete_nodes_without_type(self, types: AbstractSet[NodeTypeT]) -> None:
        self._underlying_digraph.delete_vertices(
            [i for i, n in enumerate(self.all_nodes) if n.node_type not in types]
        )
    ...
    def delete_links_without_type(self, types: AbstractSet[LinkTypeT]) -> None:
        self.delete_links(
            [
                ...
                if link.link_type not in types
            ]
        )
    ...
    def create_new(
        ...
        links: Collection[tuple[EndNodeIdPair, LinkT]],
    ) -> Self:
...
def _add_nodes(
    network: MutableNetworkABC, nodes: Mapping[NodeId, NodeT] | Collection[NodeT]
) -> None:
    ...
    nodes_to_add_iterator = (
        nodes.items() if isinstance(nodes, dict) else ((n.id, n) for n in nodes)
    )
    for i, (node_id, node) in enumerate(nodes_to_add_iterator, start=v_count_before):
        network.underlying_digraph.vs[i][node_attr_name] = node
        network.underlying_digraph.vs[i][VERTEX_NAME_IN_GRAPH] = node_id


def _add_links(
    mutable_network: MutableNetworkABC,
    links_to_add: Collection[tuple[EndNodeIdPair, LinkT]],
) -> None:
...
def _append_to_network(
    network_to_extend: MutableNetworkABC,
    network_to_append: ImmutableNetworkABC,
    ...
    if skip_duplicate_nodes:
        ...
        nodes_to_add = {
            node.id: node
            for node in network_to_append.all_nodes
        ...
        network_to_extend.add_nodes(
            {node.id: node for node in network_to_append.all_nodes}
        )
...
def _replace_node(
    network: MutableNetworkABC, index: NodeIndex, new_node: NodeT, renamed: bool
) -> None:
    if network.underlying_digraph.vs[index][VERTEX_NAME_IN_GRAPH] != new_node.id:
        ...
        assert new_node.id not in set(
            network.underlying_digraph.vs[VERTEX_NAME_IN_GRAPH]
        ), f"{new_node.id=} not unique"
...
def _delete_nodes_without_event_type(
    self: MutableNetworkABC, types: Iterable[BaseNodeType]
) -> None:
...
def _delete_nodes_with_event_type(
    self: MutableNetworkABC, types: Iterable[BaseNodeType]
) -> None:
    if not isinstance(types, (set, frozenset)):
        types = frozenset(types)
