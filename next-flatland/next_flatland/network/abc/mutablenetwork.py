...
from network.abc import ImmutableNetworkABC
from network.abc.immutablenetwork_abc import NodeT, LinkT, NodeTypeT, LinkTypeT, LinkIndex, VERTEX_NAME_IN_GRAPH
from network.abc.link import EndNodeIdPair
from network.abc.node import NodeId, NodeIndex, BaseNodeType
...
class MutableNetworkABC(ImmutableNetworkABC[NodeT, LinkT, NodeTypeT, LinkTypeT], ABC):
    ...
    def __add__(self: Self, other: Self) -> Self:
        return self.__class__(self._underlying_digraph.union(other.underlying_digraph, byname=True), self.time_context)

    def sub_network(self: Self, selected: Collection[NodeIndex] | Collection[NodeId]) -> Self:
        return self.__class__(self._underlying_digraph.subgraph(selected), self.time_context)
...
def _add_nodes(network: MutableNetworkABC, nodes: Mapping[NodeId, NodeT] | Collection[NodeT]) -> None:
    ...
    nodes_to_add_iterator = nodes.items() if isinstance(nodes, dict) else ((n.id, n) for n in nodes)
